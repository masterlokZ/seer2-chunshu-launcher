...
const CUSTOM_SKIN_RESOURCE_TYPES = new Set([
  'normal', 'fight', 'skill', 'primary', 'dictionary', 'icon', 'demo',
]);
...
function setCustomSkins(entries, metadata) {
  ...
  var nextIssuedIds = new Set(_customSkinSnapshot.issuedIds);
  var suppliedIssuedIds = metadata && Array.isArray(metadata.issuedIds) ? metadata.issuedIds : [];
  ...
  allEntries.forEach(function(entry) {
    ...
    CUSTOM_SKIN_RESOURCE_TYPES.forEach(function(type) {
      ...
      nextPreviewRoutes.set(type + ':' + skinId, { source:resolved, fallback:'' });
    });
    // A custom skin is authored as one complete battle/idle timeline. The two
    // game UIs, bag and dictionary must adapt to that timeline; the skin file
    // itself is never rewritten. Presentation requests therefore use the
    // fight package, while normal remains the lightweight map/follow model.
    // Preserve a dedicated icon. If it is absent, route the fight package;
    // PetDictionary first tries item and then freezes a visible pet frame.
  ...
  nextPreviewRoutes.forEach(function(route, key) {
    var id = parseInt(String(key).split(':').pop(), 10);
    if (activeIds.has(id)) nextRoutes.set(key, route);
  });
...
function transformCustomSkinXml(kind, input, entries) {
  ...
  skins.filter(function(entry) {
    return entry && entry.enabled !== false && parseInt(entry.skinId || entry.targetPetId, 10) > 0;
...
function _tryServeCustomSkinConfig(routeHost, routePath, replaceKey, res) {
  ...
  function finish(err, baseBuffer) {
    ...
    try {
      var output = transformCustomSkinXml(kind, baseBuffer, snapshot.entries);
      _respondCustomSkinBuffer(res, output, 'text/xml; charset=utf-8', 'CONFIG-' + kind.toUpperCase());
      console.log('[CustomSkin] transformed config:', kind, routePath, 'size=' + output.length);
    } catch(e) {
...
function _tryServeCustomSkinResource(routePath, res, includeDisabled) {
  var snapshot = _customSkinSnapshot;
  var clean = String(routePath || '').split('?')[0].replace(/\\/g, '/');
  clean = clean.replace(/^\/seer2/i, '');
  var match = clean.match(/^\/res\/pet\/(normal|fight|skill|primary|dictionary|icon|demo)\/(\d+)\.swf$/i);
...
let _renderQuality = 'high';
let _renderGpuMode = true;
...
function setRenderQuality(config) {
  ...
  if (['low','medium','high'].indexOf(quality) >= 0) _renderQuality = quality;
  if (typeof config.gpuMode === 'boolean') _renderGpuMode = config.gpuMode;
  return { quality:_renderQuality, gpuMode:_renderGpuMode };
}
...
function _applyHtmlTransform(buf) {
  ...
  // ── html/body 黑底 CSS（防止页面自身白色背景在首帧闪出）─────────────────
  // 不内嵌背景图 base64（避免每次 HTML 响应携带 100KB+ 数据）。
  var slCss  = 'html,body{background:#000!important;}';
  var slTag  = '<style id="__sl_bg">' + slCss + '</style>';
  var hmatch = body.match(/<head[^>]*>/i);
  body = hmatch ? body.replace(hmatch[0], hmatch[0] + slTag) : slTag + body;
  ...
  // 为每个 object 独立检查直属 param。不能用“整页是否出现过参数”作为条件：
  // object + embed 混合页中，PPAPI 实际采用的 embed 可能因此拿不到配置。
  ...
  // GPU 模式使用 Flash 的 direct 合成路径；兼容模式退回 window。
  var wmode = _renderGpuMode ? 'direct' : 'window';
  body = body.replace(/<embed\b[^>]*>/gi, function(tag) {
    return setTagAttribute(tag, 'wmode', wmode);
  });
  body = ensureObjectParam(body, 'wmode', wmode);
  // 画质可在面板中动态切换，默认 high。
  var quality = _renderQuality;
  body = body.replace(/<embed\b[^>]*>/gi, function(tag) {
    return setTagAttribute(tag, 'quality', quality);
  });
  body = ensureObjectParam(body, 'quality', quality);
  // ── hasPriority=true ──────────────────────────────────────────────────────
  if (body.indexOf('hasPriority') === -1) {
    body = body.replace(/(<embed\s[^>]*type\s*=\s*["\'"]?application\/x-shockwave-flash["\'"]?[^>]*?)(\/?\s*>)/gi,
      (m, open, close) => open + ' hasPriority="true"' + close);
    body = body.replace(/(<object\s[^>]*(?:classid|type)[^>]*>)/gi,
      m => m + '<param name="hasPriority" value="true"/>');
  }
...
function startInterceptProxy() {
  return new Promise(function(resolve, reject) {
    const srv = http.createServer(function(req, res) {
      ...
      var previewModelMatch = (host === 'seer.61.com' || host === '127.0.0.1') && reqPath.match(/^\/launcher\/pet-preview\/(fight|normal|icon|skill)\/(\d+)\.swf$/);
      if (previewModelMatch) {
        var previewKind = previewModelMatch[1];
        var previewId = previewModelMatch[2];
        // Downloaded/custom skins are registered under a safe local skin id.
        // Let the preview player use the exact same snapshot route as the
        // game, so its animation buttons inspect the user's actual files
        // instead of downloading an unrelated official resource with that id.
        if (_tryServeCustomSkinResource('/res/pet/' + previewKind + '/' + previewId + '.swf', res, true)) {
          return;
        }
