...
function buildIndex(packages) {
  ...
  battle.assets.forEach(function(asset) {
    ...
    let match = value.match(/^Assets\/Pets\/(\d+)\.prefab$/i);
    if (match) { if (Number(match[1]) > 0) ftr.set(Number(match[1]),asset); return; }
    match = value.match(/^Assets\/Pets\/(\d+)_small\.prefab$/i);
    if (match) { if (Number(match[1]) > 0) small.set(Number(match[1]),asset); return; }
    match = value.match(/^Assets\/SkillTimeline\/Spines\/(\d+)\/\1\.prefab$/i);
    if (match) { if (Number(match[1]) > 0) spine.set(Number(match[1]),asset); return; }
