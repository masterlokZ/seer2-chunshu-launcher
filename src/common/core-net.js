...
const customSkinManifests = require('./modules/custom-skin-route-manifest');
...
let _uclientPetPreviewResolver = null;
...
// ── 自定义精灵皮肤系统 ────────────────────────────────────────────────────
// 游戏内 PetSkinConfig 已经负责“当前精灵 -> 皮肤资源序号”的选择。
// 启动器只登记外部皮肤资源序号，不再要求用户指定它属于哪只精灵：
//   1. 六类 SWF 按 skinId 路由到本地文件或远程 URL；
//   2. 45/64/3 中已有该序号时完全保留服务器定义；
//   3. 新序号不存在时，动态补齐图鉴、基础定义和安全的出伤帧记录。
const CUSTOM_SKIN_RESOURCE_TYPES = new Set([
  'normal', 'fight', 'physical', 'special', 'property', 'skill', 'primary', 'dictionary', 'icon', 'demo',
]);
const CUSTOM_SKIN_CONFIG_FILES = {
  '45_com.taomee.seer2.app.config.petconfig__dictionaryxmlclass.xml': 'dictionary',
  '64_com.taomee.seer2.app.config.petconfig__petxmlclass.xml': 'pet',
  '501_com.taomee.seer2.app.config.petskinconfig__xmlclass.xml': 'skin',
  '3_com.taomee.seer2.app.arena.util.hitinfoconfig__hitdata.xml': 'hit',
};
let _customSkins = [];
let _customSkinResourceMap = new Map(); // "type:skinId" -> {source,fallback}
let _customSkinSnapshot = Object.freeze({
  revision:0,
  entries:Object.freeze([]),
  routes:new Map(),
  previewRoutes:new Map(),
  issuedIds:new Set(),
});
const CUSTOM_SKIN_CONFIG_CACHE_LIMIT = 12;
let _customSkinConfigTransformCache = new Map();
// 皮肤 SWF 在图鉴、背包和战斗切场时可能被连续请求多次。过去每次都重新
// readFile，即便是同一份 30MB+ 模型也会反复触发磁盘 IO、Buffer 分配和 GC。
// 使用有上限的 LRU，并合并同一路径的并发读取；注册快照变化时整体失效。
const CUSTOM_SKIN_LOCAL_CACHE_MAX = 192 * 1024 * 1024;
const CUSTOM_SKIN_LOCAL_CACHE_ITEM_MAX = 64 * 1024 * 1024;
let _customSkinLocalCache = new Map();
let _customSkinLocalCacheBytes = 0;
let _customSkinLocalReads = new Map();
let _customSkinLocalCacheGeneration = 0;

function _clearCustomSkinLocalCache() {
  _customSkinLocalCache.clear();
  _customSkinLocalCacheBytes = 0;
  _customSkinLocalCacheGeneration++;
}

function setUClientPetPreviewResolver(resolver) {
  _uclientPetPreviewResolver = typeof resolver === 'function' ? resolver : null;
}

function _readCustomSkinLocal(source, callback) {
  var key = path.resolve(source).toLowerCase();
  var generation = _customSkinLocalCacheGeneration;
  var readKey = generation + '|' + key;
  var hit = _customSkinLocalCache.get(key);
  if (hit) {
    _customSkinLocalCache.delete(key);
    _customSkinLocalCache.set(key, hit);
    process.nextTick(function() { callback(null, hit.buffer, true); });
    return;
  }
  var waiting = _customSkinLocalReads.get(readKey);
  if (waiting) { waiting.push(callback); return; }
  _customSkinLocalReads.set(readKey, [callback]);
  fs.readFile(source, function(err, buffer) {
    var callbacks = _customSkinLocalReads.get(readKey) || [];
    _customSkinLocalReads.delete(readKey);
    if (generation === _customSkinLocalCacheGeneration && !err && buffer && buffer.length && buffer.length <= CUSTOM_SKIN_LOCAL_CACHE_ITEM_MAX) {
      while (_customSkinLocalCacheBytes + buffer.length > CUSTOM_SKIN_LOCAL_CACHE_MAX && _customSkinLocalCache.size) {
        var oldestKey = _customSkinLocalCache.keys().next().value;
        var oldest = _customSkinLocalCache.get(oldestKey);
        _customSkinLocalCache.delete(oldestKey);
        _customSkinLocalCacheBytes -= oldest.buffer.length;
      }
      _customSkinLocalCache.set(key, { buffer:buffer });
      _customSkinLocalCacheBytes += buffer.length;
    }
    callbacks.forEach(function(done) { done(err, buffer, false); });
  });
}

function setCustomSkins(entries, metadata) {
  var allEntries = Array.isArray(entries) ? entries.filter(function(entry) {
    return entry && parseInt(entry.skinId || entry.targetPetId, 10) > 0;
  }) : [];
  var nextEntries = allEntries.filter(function(entry) { return entry.enabled !== false; });
  var nextPreviewRoutes = new Map();
  var hasAuthoritativeIssuedIds = !!(metadata &&
    Object.prototype.hasOwnProperty.call(metadata, 'issuedIds') &&
    Array.isArray(metadata.issuedIds));
  var nextIssuedIds = hasAuthoritativeIssuedIds
    ? new Set()
    : new Set(_customSkinSnapshot.issuedIds);
  var suppliedIssuedIds = hasAuthoritativeIssuedIds ? metadata.issuedIds : [];
  suppliedIssuedIds.forEach(function(value) {
    var id = parseInt(value, 10);
    if (id > 0) nextIssuedIds.add(id);
  });
  allEntries.forEach(function(entry) {
    var skinId = parseInt(entry.skinId || entry.targetPetId, 10);
    var officialIdOverride = entry.officialIdOverride === true;
    if (officialIdOverride) nextIssuedIds.delete(skinId);
    else if (!hasAuthoritativeIssuedIds) nextIssuedIds.add(skinId);
    var files = entry.files || {};
    CUSTOM_SKIN_RESOURCE_TYPES.forEach(function(type) {
      var source = String(files[type] || '').trim();
      if (!source) return;
      var resolved = _resolveReplaceFile(source);
      // Exact registered ID always owns its resource.  A same-name built-in
      // skin must never replace the local battle package.
      nextPreviewRoutes.set(type + ':' + skinId,
        { source:resolved, fallback:'', officialIdOverride:officialIdOverride });
    });
    // U-client downloads expose a synchronous, self-contained fight.swf.
    if (entry.previewAdapter === 'uclient') {
      var uclientFightSource = String(files.fight || '').trim();
      if (uclientFightSource) nextPreviewRoutes.set('fight:' + skinId, {
        source:_resolveReplaceFile(uclientFightSource), fallback:'', officialIdOverride:officialIdOverride,
      });
    }
    var fightSource = String(files.fight || '').trim();
    if (fightSource && entry.presentationMode !== 'legacy') {
      ['demo','primary','dictionary'].forEach(function(type) {
        var key = type + ':' + skinId;
        if (nextPreviewRoutes.has(key)) return;
        nextPreviewRoutes.set(key, {
          source:_resolveReplaceFile(fightSource),
          fallback:'',
          presentation:'full-idle',
          officialIdOverride:officialIdOverride,
        });
      });
      var i = String(files.icon || '').trim();
      var f = _resolveReplaceFile(fightSource);
      var r = i ? _resolveReplaceFile(i) : '';
      var sameIconFile = customSkinManifests.isSameFile(r, f);
      if (!officialIdOverride && customSkinManifests.isFightDerivedIconRoute(entry,
        nextPreviewRoutes.get('icon:' + skinId), nextPreviewRoutes.get('fight:' + skinId), sameIconFile)) {
        nextPreviewRoutes.set('icon:' + skinId, {
          source:r || f,
          fallback:'',
          presentation:'frozen-avatar',
          officialIdOverride:officialIdOverride,
        });
      }
    }
    // Legacy entries without a fight presentation retain their normal model.
    var modelSource = String(files.normal || '').trim();
    if (modelSource) {
      ['demo','primary','dictionary'].forEach(function(type) {
        var key = type + ':' + skinId;
        if (!nextPreviewRoutes.has(key)) {
          nextPreviewRoutes.set(key, { source:_resolveReplaceFile(modelSource), fallback:'',
            officialIdOverride:officialIdOverride });
        }
      });
    }
  });
  var activeIds = new Set(nextEntries.map(function(entry) {
    return parseInt(entry.skinId || entry.targetPetId, 10);
  }));
  var nextRoutes = new Map();
  nextPreviewRoutes.forEach(function(route, key) {
    var parts = String(key).split(':');
    var type = parts[0];
    var id = parseInt(parts[parts.length - 1], 10);
    if (activeIds.has(id) && !(route.officialIdOverride === true && type === 'icon')) {
      nextRoutes.set(key, route);
    }
  });
  var nextRevision = metadata && parseInt(metadata.revision, 10) > 0
    ? parseInt(metadata.revision, 10)
    : (_customSkinSnapshot.revision + 1);
  // Build first, then swap all runtime state together. In-flight requests retain
  // the snapshot they captured and can never observe a half-cleared route table.
  _customSkinSnapshot = Object.freeze({
    revision:nextRevision,
    entries:Object.freeze(nextEntries.slice()),
    routes:nextRoutes,
    previewRoutes:nextPreviewRoutes,
    issuedIds:nextIssuedIds,
  });
  _customSkins = _customSkinSnapshot.entries;
  _customSkinResourceMap = _customSkinSnapshot.routes;
  _clearCustomSkinLocalCache();
  // A snapshot change can reuse the same externally supplied revision, so
  // invalidate transformed XML explicitly instead of relying on the key alone.
  _customSkinConfigTransformCache.clear();
  console.log('[CustomSkin] snapshot:', nextRevision, _customSkins.length,
    'entries,', _customSkinResourceMap.size, 'routes,', nextIssuedIds.size, 'issued ids');
}

function getCustomSkins() {
  return _customSkins.slice();
}

function getCustomSkinSnapshotInfo() {
  return {
    revision:_customSkinSnapshot.revision,
    entryCount:_customSkinSnapshot.entries.length,
    routeCount:_customSkinSnapshot.routes.size,
    previewRouteCount:_customSkinSnapshot.previewRoutes.size,
    issuedIds:Array.from(_customSkinSnapshot.issuedIds).sort(function(a,b) { return a - b; }),
  };
}

// Read-only bridge for game modules. Membership comes exclusively from the
// launcher's active registry snapshot; game XML fields and numeric ranges are
// deliberately irrelevant here. PetSkinPanel can therefore preserve the
// current pet's official list and append only enabled launcher skins.
function buildCustomSkinManifestXml() {
  return customSkinManifests.buildCustomSkinManifestXml(_customSkinSnapshot);
}

function buildCustomSkinRoutesXml() {
  return customSkinManifests.buildCustomSkinRoutesXml(_customSkinSnapshot);
}

function buildPetSkinPanelRoutesXml() {
  return customSkinManifests.buildPetSkinPanelRoutesXml(_customSkinSnapshot);
}

function _tryServeCustomSkinManifest(routePath, res) {
  var clean = String(routePath || '').split('?')[0].replace(/\\/g, '/').toLowerCase();
  var isRoutes = /\/launcher\/custom-skin-routes\.xml$/.test(clean);
  var isPetSkinPanelRoutes = /\/launcher\/pet-skin-panel-routes\.xml$/.test(clean);
  if (!isRoutes && !isPetSkinPanelRoutes && !/\/launcher\/custom-skins\.xml$/.test(clean)) return false;
  var output = isPetSkinPanelRoutes ? buildPetSkinPanelRoutesXml()
    : (isRoutes ? buildCustomSkinRoutesXml() : buildCustomSkinManifestXml());
  _respondCustomSkinBuffer(res, output, 'text/xml; charset=utf-8',
    isPetSkinPanelRoutes ? 'PETSKIN-PANEL-ROUTE-MANIFEST' : (isRoutes ? 'ROUTE-MANIFEST' : 'MANIFEST'));
  return true;
}
...
function _setXmlAttribute(tag, name, value) {
  if (value == null || value === '') return tag;
  var escaped = _escapeXmlText(value);
  var re = new RegExp('(\\s' + name + '\\s*=\\s*)([\"\\\'])(.*?)\\2', 'i');
  if (re.test(tag)) {
    return tag.replace(re, function(_all, prefix, quote) {
...
function _patchXmlStartTag(xml, tagName, idAttr, idValue, updater) {
  var id = parseInt(idValue, 10);
  if (!id) return xml;
  var re = new RegExp(
    '<' + tagName + '\\b(?=[^>]*\\b' + idAttr + '\\s*=\\s*([\"\\\'])' + id + '\\1)[^>]*>',
    'i'
  );
  return xml.replace(re, function(tag) { return updater(tag); });
}

function _hasXmlEntry(xml, tagName, idAttr, idValue) {
  var id = parseInt(idValue, 10);
  if (!id) return false;
  var re = new RegExp(
    '<' + tagName + '\\b(?=[^>]*\\b' + idAttr + '\\s*=\\s*([\"\\\'])' + id + '\\1)[^>]*>',
...
function _findXmlStartTag(xml, tagName, idAttr, idValue) {
  var id = parseInt(idValue, 10);
  if (!id) return '';
  var re = new RegExp(
    '<' + tagName + '\\b(?=[^>]*\\b' + idAttr + '\\s*=\\s*([\"\\\'])' + id + '\\1)[^>]*>',
  ...
  var match = String(xml || '').match(re);
  return match ? match[0] : '';
}

function _insertBeforeRootClose(xml, closingTag, node) {
  var lower = xml.toLowerCase();
  var marker = '</' + String(closingTag || '').toLowerCase();
  var index = lower.lastIndexOf(marker);
  if (index >= 0) return xml.slice(0, index) + '\n' + node + '\n' + xml.slice(index);
  return xml + '\n' + node + '\n';
}

function _transformCustomSkinXmlLegacy(kind, input, entries) {
  var xml = Buffer.isBuffer(input) ? input.toString('utf8') : String(input || '');
  var skins = Array.isArray(entries) ? entries : _customSkins;
  skins.filter(function(entry) {
    // Resolve occupancy from the actual XML below.  An official override can
    // still target an unused gap in an older registry and must receive a
    // synthetic definition there.
    return entry && entry.enabled !== false &&
      parseInt(entry.skinId || entry.targetPetId, 10) > 0;
  }).forEach(function(entry) {
    var skinId = parseInt(entry.skinId || entry.targetPetId, 10);
    var name = String(entry.name || ('本地皮肤 ' + skinId)).trim();
    if (kind === 'dictionary') {
      if (!_hasXmlEntry(xml, 'Monster', 'ID', skinId)) {
        xml = _insertBeforeRootClose(xml, 'root',
          '    <Monster NumbersID="' + skinId + '" emblemID="000000" ID="' + skinId +
          '" DefName="' + _escapeXmlText(name) +
          '" Type="皮肤" Height="0 0" Weight="0 0" Foundin="启动器本地导入" isNew="0"' +
          ' Features="可在游戏皮肤功能中输入序号 ' + skinId +
          '" intro="由启动器自动登记的外部 SWF 皮肤" chara="自定义资源"/>');
      }
    } else if (kind === 'pet') {
      if (!_hasXmlEntry(xml, 'Monster', 'ID', skinId)) {
        xml = _insertBeforeRootClose(xml, 'Monsters',
          '    <Monster NumbersID="' + skinId + '" ID="' + skinId +
          '" DefName="' + _escapeXmlText(name) +
          '" Type="24" GrowthType="0" HP="1" Atk="1" Def="1" SpAtk="1" SpDef="1" Spd="1"' +
          ' YieldingExp="0" CatchRate="0" YieldingEV="0" EvolvesFrom="0" EvolvesTo="0"' +
          ' Height="0 0" Weight="0 0" Features="9999" Gender="0 10" FreeForbidden="1" StarLv="1">' +
          '\n        <LearnableMoves></LearnableMoves>\n    </Monster>');
      }
    } else if (kind === 'hit') {
      if (!_hasXmlEntry(xml, 'fighter', 'id', skinId)) {
        var sourceId = parseInt(entry.sourceId, 10);
        var sourceHitTag = sourceId ? _findXmlStartTag(xml, 'fighter', 'id', sourceId) : '';
        var hitNode = sourceHitTag
          ? _setXmlAttribute(sourceHitTag, 'id', skinId)
          : '<fighter id="' + skinId +
            '" physics="20" attribute="20" special="20" critical="20" fit="20" />';
        xml = _insertBeforeRootClose(xml, 'fighters',
          '\t' + hitNode);
      }
    }
  });
...
function _indexXmlStartTags(xml, tagName, idAttr) {
  var ids = new Set();
  var tags = new Map();
  var tagRe = new RegExp('<' + tagName + '\\b[^>]*>', 'gi');
  var idRe = new RegExp("\\b" + idAttr + "\\s*=\\s*([\\\"'])(\\d+)\\1", 'i');
  var match;
  while ((match = tagRe.exec(xml)) !== null) {
    var idMatch = match[0].match(idRe);
    if (!idMatch) continue;
    var id = parseInt(idMatch[2], 10);
    if (!id) continue;
    ids.add(id);
    if (!tags.has(id)) tags.set(id, match[0]);
  }
  return { ids:ids, tags:tags };
}

function _insertNodesBeforeRootClose(xml, closingTag, nodes) {
  if (!Array.isArray(nodes) || nodes.length === 0) return xml;
  var lower = xml.toLowerCase();
  var marker = '</' + String(closingTag || '').toLowerCase();
  var index = lower.lastIndexOf(marker);
  var block = '\n' + nodes.join('\n') + '\n';
  if (index >= 0) return xml.slice(0, index) + block + xml.slice(index);
  return xml + block;
}

// Scan the base XML once, build all missing nodes, then copy the document once.
// The old implementation rescanned and recopied the growing document per skin.
function transformCustomSkinXml(kind, input, entries) {
  var xml = Buffer.isBuffer(input) ? input.toString('utf8') : String(input || '');
  var skins = Array.isArray(entries) ? entries : _customSkins;
  var activeSkins = skins.filter(function(entry) {
    return entry && entry.enabled !== false &&
      parseInt(entry.skinId || entry.targetPetId, 10) > 0;
  });
  if (activeSkins.length === 0) return Buffer.from(xml, 'utf8');

  var tagName = kind === 'hit' ? 'fighter' : (kind === 'skin' ? 'pet' : 'Monster');
  var idAttr = kind === 'hit' ? 'id' : (kind === 'skin' ? 'resourceId' : 'ID');
  var indexed = _indexXmlStartTags(xml, tagName, idAttr);
  var nodes = [];

  activeSkins.forEach(function(entry) {
    var skinId = parseInt(entry.skinId || entry.targetPetId, 10);
    if (indexed.ids.has(skinId)) return;
    var name = String(entry.name || ('本地皮肤 ' + skinId)).trim();

    if (kind === 'dictionary') {
      nodes.push('    <Monster NumbersID="' + skinId + '" emblemID="000000" ID="' + skinId +
        '" DefName="' + _escapeXmlText(name) +
        '" Type="皮肤" Height="0 0" Weight="0 0" Foundin="启动器本地导入" isNew="0"' +
        ' Features="可在游戏皮肤功能中输入序号 ' + skinId +
        '" intro="由启动器自动登记的外部 SWF 皮肤" chara="自定义资源"/>');
    } else if (kind === 'pet') {
      nodes.push('    <Monster NumbersID="' + skinId + '" ID="' + skinId +
        '" DefName="' + _escapeXmlText(name) +
        '" Type="24" GrowthType="0" HP="1" Atk="1" Def="1" SpAtk="1" SpDef="1" Spd="1"' +
        ' YieldingExp="0" CatchRate="0" YieldingEV="0" EvolvesFrom="0" EvolvesTo="0"' +
        ' Height="0 0" Weight="0 0" Features="9999" Gender="0 10" FreeForbidden="1" StarLv="1">' +
      ...
      // PetSkinConfig is the binding that makes a runtime skin slot
      // selectable for its source pet.  When a launcher id lands in an
      // otherwise empty slot, add only that binding; an existing binding is
      // left untouched so occupied official ids retain their original skin.
      var resourceId = parseInt(entry.basePetId, 10) || parseInt(entry.sourceId, 10) || skinId;
      var skinTag = indexed.tags.get(resourceId) || '';
      if (skinTag) {
        var existingSkinId = skinTag.match(/\bskinId\s*=\s*(["'])(\d+)\1/i);
        var existingValue = existingSkinId ? parseInt(existingSkinId[2], 10) : 0;
        if (existingValue === skinId || existingValue > 0) return;
        var replacementTag = _setXmlAttribute(skinTag, 'skinId', skinId);
        xml = xml.replace(skinTag, replacementTag);
        indexed.tags.set(resourceId, replacementTag);
        return;
      }
      nodes.push('    <pet resourceId="' + resourceId + '" skinId="' + skinId + '"/>');
      indexed.tags.set(resourceId, nodes[nodes.length - 1]);
    } else if (kind === 'hit') {
      var sourceId = parseInt(entry.sourceId, 10);
      var sourceHitTag = sourceId ? (indexed.tags.get(sourceId) || '') : '';
      var hitNode = sourceHitTag
        ? _setXmlAttribute(sourceHitTag, 'id', skinId)
        : '<fighter id="' + skinId +
          '" physics="20" attribute="20" special="20" critical="20" fit="20" />';
      nodes.push('\t' + hitNode);
      indexed.tags.set(skinId, hitNode);
    } else {
      return;
    }
    indexed.ids.add(skinId);
  });

  var closingTag = kind === 'dictionary' ? 'root'
    : (kind === 'pet' ? 'Monsters' : (kind === 'skin' ? 'pets' : 'fighters'));
  return Buffer.from(_insertNodesBeforeRootClose(xml, closingTag, nodes), 'utf8');
}

function _getCachedCustomSkinConfig(cacheKey) {
  var output = _customSkinConfigTransformCache.get(cacheKey);
  if (!output) return null;
  _customSkinConfigTransformCache.delete(cacheKey);
  _customSkinConfigTransformCache.set(cacheKey, output);
  return output;
}

function _cacheCustomSkinConfig(cacheKey, output) {
  _customSkinConfigTransformCache.set(cacheKey, output);
  while (_customSkinConfigTransformCache.size > CUSTOM_SKIN_CONFIG_CACHE_LIMIT) {
    var oldestKey = _customSkinConfigTransformCache.keys().next().value;
    _customSkinConfigTransformCache.delete(oldestKey);
  }
}

function _customSkinConfigKind(reqPath) {
  var clean = String(reqPath || '').split('?')[0].replace(/\\/g, '/').toLowerCase();
  var file = clean.slice(clean.lastIndexOf('/') + 1);
  return CUSTOM_SKIN_CONFIG_FILES[file] || '';
}

function _decodeResponseBuffer(buffer, encoding, callback) {
  var enc = String(encoding || '').toLowerCase();
  if (enc === 'gzip') return zlib.gunzip(buffer, callback);
  if (enc === 'deflate') return zlib.inflate(buffer, callback);
  if (enc === 'br' && zlib.brotliDecompress) return zlib.brotliDecompress(buffer, callback);
  callback(null, buffer);
}

function _fetchSourceBuffer(sourceUrl, redirectCount, callback) {
  var count = redirectCount || 0;
  var parsed;
  try { parsed = urlMod.parse(sourceUrl); } catch(e) { callback(e); return; }
  if (!parsed || !parsed.hostname || !/^https?:$/i.test(parsed.protocol || '')) {
    callback(new Error('invalid source URL'));
    return;
  }
  var isHttps = String(parsed.protocol).toLowerCase() === 'https:';
  var transport = isHttps ? require('https') : http;
  var agent = isHttps ? (_proxyAgent || _fwdHttpsAgent) : _getOutboundAgent();
  var request = transport.request({
    ...
    method: 'GET',
    headers: {
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/87 Safari/537.36',
    ...
    if (status >= 300 && status < 400 && response.headers.location && count < 4) {
      var nextUrl = urlMod.resolve(sourceUrl, response.headers.location);
      response.resume();
      _fetchSourceBuffer(nextUrl, count + 1, callback);
      return;
    }
    var chunks = [];
    response.on('data', function(chunk) { if (chunk && chunk.length) chunks.push(chunk); });
    response.on('end', function() {
      ...
      _decodeResponseBuffer(raw, response.headers['content-encoding'], callback);
    });
...
function _respondCustomSkinBuffer(res, buffer, mime, type) {
  res.writeHead(200, {
    'Content-Type': mime,
    ...
    'Connection': 'close',
    'X-Seer2-Custom-Skin': type,
  });
  res.end(buffer);
}

function _tryServeCustomSkinConfig(routeHost, routePath, replaceKey, res) {
  var snapshot = _customSkinSnapshot;
  if (snapshot.entries.length === 0) return false;
  var kind = _customSkinConfigKind(routePath);
  if (!kind) return false;

  var replaceEntry = _userReplaceCache.get(replaceKey);
  function finish(err, baseBuffer) {
    if (err || !baseBuffer || !baseBuffer.length) {
      console.warn('[CustomSkin] config base fetch failed:', routePath, err && err.message);
      res.writeHead(502, {
      ...
      res.end('Custom skin config source failed');
      return;
    }
    try {
      var baseHash = crypto.createHash('sha256').update(baseBuffer).digest('hex');
      var cacheKey = snapshot.revision + '|' + kind + '|' + baseHash;
      var output = _getCachedCustomSkinConfig(cacheKey);
      var cacheState = 'hit';
      if (!output) {
        output = transformCustomSkinXml(kind, baseBuffer, snapshot.entries);
        _cacheCustomSkinConfig(cacheKey, output);
        cacheState = 'miss';
      }
      _respondCustomSkinBuffer(res, output, 'text/xml; charset=utf-8', 'CONFIG-' + kind.toUpperCase());
      console.log('[CustomSkin] transformed config:', kind, routePath,
        'size=' + output.length, 'cache=' + cacheState);
    } catch(e) {
      console.warn('[CustomSkin] config transform failed:', routePath, e.message);
      res.writeHead(500, { 'Content-Type':'text/plain; charset=utf-8', 'Connection':'close' });
      res.end('Custom skin config transform failed');
    }
  }

  if (replaceEntry && replaceEntry.kind === 'file') {
    fs.readFile(replaceEntry.file, finish);
    return true;
  }
  var sourceUrl = replaceEntry && replaceEntry.kind === 'network'
    ? replaceEntry.url
    : ('http://' + routeHost + routePath);
  _fetchSourceBuffer(sourceUrl, 0, finish);
  return true;
}

function _tryServeCustomSkinResource(routePath, res, includeDisabled, officialOverridesOnly) {
  var snapshot = _customSkinSnapshot;
  var clean = String(routePath || '').split('?')[0].replace(/\\/g, '/');
  clean = clean.replace(/^\/seer2/i, '');
  var match = clean.match(/^\/res\/pet\/(normal|fight|physical|special|property|skill|primary|dictionary|icon|demo)\/(\d+)\.swf$/i);
  if (!match) return false;
  var type = match[1].toLowerCase();
  var petId = parseInt(match[2], 10);
  var routeMap = includeDisabled === true ? snapshot.previewRoutes : snapshot.routes;
  var route = routeMap.get(type + ':' + petId);
  if (officialOverridesOnly === true && (!route || route.officialIdOverride !== true)) {
    return false;
  }
  if (!route) {
    // 跟随显示会优先探测真正的 normal 地图模型。对已登记但未提供
    // normal 的自定义皮肤在本地立即返回 404，让 CoreDLL 无延迟回退到
    // demo 静态模型；不要把不存在的自定义序号资源请求转发给游戏服务器。
    // A synthetic id that existed in any committed revision must never leak to
    // the game server. Flash may retain an older injected XML singleton until
    // reload; answering locally avoids the historical 20s bag freeze.
    if (snapshot.issuedIds.has(petId)) {
      res.writeHead(410, {
        'Content-Type':'text/plain; charset=utf-8',
        'Cache-Control':'no-store',
        'Connection':'close',
        'X-Seer2-Custom-Skin':'RETIRED',
      });
      res.end('Custom skin resource is no longer registered');
      return true;
    }
    return false;
  }

  var source = String(route.source || route || '').trim();
  var fallback = String(route.fallback || '').trim();

  function serveLocal(localSource, routeType) {
    _readCustomSkinLocal(localSource, function(err, buffer) {
      if (err || !buffer || !buffer.length) {
        console.warn('[CustomSkin] local SWF failed:', type, petId, localSource,
          err && (err.code || err.message));
        if (!res.headersSent) {
          res.writeHead(404, { 'Content-Type':'text/plain; charset=utf-8', 'Connection':'close' });
        }
        res.end('Custom skin local SWF not found');
        return;
      }
      _respondCustomSkinBuffer(res, buffer, 'application/x-shockwave-flash', routeType);
    });
  }

  if (/^https?:\/\//i.test(source)) {
    _fetchSourceBuffer(source, 0, function(err, buffer) {
      if (err || !buffer || !buffer.length) {
        console.warn('[CustomSkin] remote SWF failed:', type, petId, source, err && err.message);
        if (fallback && !/^https?:\/\//i.test(fallback)) {
          serveLocal(fallback, 'LOCAL-FALLBACK-' + type.toUpperCase());
        } else {
          res.writeHead(502, { 'Content-Type':'text/plain; charset=utf-8', 'Connection':'close' });
          res.end('Custom skin remote SWF failed');
        }
        return;
      }
      _respondCustomSkinBuffer(res, buffer, 'application/x-shockwave-flash', 'REMOTE-' + type.toUpperCase());
      console.log('[CustomSkin] remote SWF:', type, petId, '->', source, 'size=' + buffer.length);
    });
    return true;
  }

  // 大型战斗皮肤常有十几至几十 MiB。readFileSync 会堵住 Electron 主线程，
  // 表现为用户第一次点击游戏窗口时整台登录器短暂无响应；改为 libuv 异步读取。
  serveLocal(source, 'LOCAL-' + type.toUpperCase());
  return true;
}

// PetSkinPanel's LRU avatar path keeps local skin identity isolated from the
// normal pet-icon route. Prefer a genuine local icon and fall back to a local
// fight/normal/demo model when no native portrait exists.
function _tryServePetSkinPanelAvatar(routePath, res) {
  var clean = String(routePath || '').split('?')[0].replace(/\\/g, '/');
  clean = clean.replace(/^\/seer2/i, '');
  var match = clean.match(/^\/launcher\/pet-skin-panel-avatar\/(\d+)\.swf$/i);
  if (!match) return false;

  var petId = parseInt(match[1], 10);
  var snapshot = _customSkinSnapshot;
  var activeEntry = snapshot.entries.find(function(entry) {
    return parseInt(entry && (entry.skinId || entry.targetPetId), 10) === petId;
  });
  if (!activeEntry) {
    res.writeHead(404, { 'Content-Type':'text/plain; charset=utf-8', 'Cache-Control':'no-store', 'Connection':'close' });
    res.end('PetSkinPanel skin is not active');
    return true;
  }

  var iconRoute = snapshot.previewRoutes.get('icon:' + petId);
  var fightRoute = snapshot.previewRoutes.get('fight:' + petId);
  var sameIconFile = customSkinManifests.isSameFile(iconRoute && iconRoute.source, fightRoute && fightRoute.source);
  var genuineIcon = !!iconRoute && !customSkinManifests.isFightDerivedIconRoute(activeEntry, iconRoute, fightRoute, sameIconFile);
  var kinds = genuineIcon ? ['icon','fight','normal','demo','primary','dictionary']
    : ['fight','normal','demo','primary','dictionary'];
  var selected = '';
  for (var index = 0; index < kinds.length; index++) {
    if (snapshot.previewRoutes.has(kinds[index] + ':' + petId)) {
  ...
  if (!selected) {
    res.writeHead(404, { 'Content-Type':'text/plain; charset=utf-8', 'Cache-Control':'no-store', 'Connection':'close' });
    res.end('PetSkinPanel avatar resource is unavailable');
    return true;
  }
  return _tryServeCustomSkinResource('/res/pet/' + selected + '/' + petId + '.swf', res, true);
}
...
function loadFilesIntoMemory(baseDir) {
  const targets = Object.values(INTERCEPT_PATHS).map(e => e.file)
    .concat(['qq-group.jpg', 'bilibili.jpg', 'contact-qq.jpg', 'skin-preview.swf']);
...
function startInterceptProxy() {
  return new Promise(function(resolve, reject) {
    const srv = http.createServer(function(req, res) {
      ...
      // 所有用户规则与皮肤配置共用同一套跨线路规范化键。
      var _replFullUrl = 'http://' + (routeHostForOps || '') + routePathForOps;
      var _replKey = _normalizeUrl(_normalizeCacheUrl(_replFullUrl));

      // Isolated true-identity avatar route for the in-game PetSkinPanel.
      if (_tryServePetSkinPanelAvatar(routePathForOps, res)) {
        return;
      }

      // Same-origin read-only manifest for the in-game skin panel. It exposes
      // only the currently enabled launcher registry, never official entries.
      if (_tryServeCustomSkinManifest(routePathForOps, res)) {
        return;
      }

      // ── 自定义皮肤 XML（45/64/3）──────────────────────────────────────
      // 必须先于普通规则处理：若用户已经为 45 等 XML 配置了替换文件，
      // 皮肤系统会以该文件为基底继续注入，而不是把两套功能互相覆盖。
      if (_tryServeCustomSkinConfig(routeHostForOps, routePathForOps, _replKey, res)) {
        return;
      }

      // UClient 动画图集由登录器内置服务读取。渲染页面不直接访问 file://，
      // 因此安装路径含中文、空格或关闭 GPU 时都不影响 Canvas 播放。
      var uclientAtlasMatch = (host === 'seer.61.com' || host === '127.0.0.1') &&
        reqPath.match(/^\/launcher\/uclient-pet-preview\/(\d+)\/atlas\.(?:webp|png)$/);
      if (uclientAtlasMatch) {
        var uclientAtlasFile = null;
        try { uclientAtlasFile = _uclientPetPreviewResolver && _uclientPetPreviewResolver(parseInt(uclientAtlasMatch[1], 10)); } catch(_) {}
        if (!uclientAtlasFile || !fs.existsSync(uclientAtlasFile)) {
          res.writeHead(404, { 'Content-Type':'text/plain; charset=utf-8', 'Cache-Control':'no-store', 'Access-Control-Allow-Origin':'*' });
          res.end('UClient animation atlas unavailable');
          return;
        }
        try {
          var uclientAtlasStat = fs.statSync(uclientAtlasFile);
          var uclientAtlasMime = /\.png$/i.test(uclientAtlasFile) ? 'image/png' : 'image/webp';
          res.writeHead(200, {
            'Content-Type':uclientAtlasMime,
            'Content-Length':uclientAtlasStat.size,
            'Cache-Control':'private, max-age=300',
            'Access-Control-Allow-Origin':'*',
          });
          if (req.method === 'HEAD') res.end();
          else fs.createReadStream(uclientAtlasFile).on('error', function() { if (!res.writableEnded) res.end(); }).pipe(res);
        } catch(error) {
      ...
      // 皮肤资料库的按需动画预览器。它使用 seer.61.com 的同源地址加载官方
      // 模型，避免跨域脚本限制；只有打开预览时才创建一个 PPAPI 实例。
      if ((host === 'seer.61.com' || host === '127.0.0.1') && reqPath === '/launcher/skin-preview.swf') {
        const preview = _fileCache.get('skin-preview.swf');
        if (preview) {
          res.writeHead(200, {
          ...
          res.writeHead(404); res.end('preview player unavailable');
        }
        return;
      }

      // 只读、限路径的官方模型同源转发。官方 HTTP 会 301 到 HTTPS，而 Flash
      // 会把协议变化视为另一安全域，无法读取其中导出的 pet 类；这里在后台取回
      // 官方 HTTPS 原文件，再从与预览器相同的 seer.61.com HTTP 源返回。
      var previewModelMatch = (host === 'seer.61.com' || host === '127.0.0.1') && reqPath.match(/^\/launcher\/pet-preview(-official)?\/(fight|normal|physical|special|property|icon|skill)\/(\d+)\.swf$/);
      if (previewModelMatch) {
        var previewOfficialOnly = previewModelMatch[1] === '-official';
        var previewKind = previewModelMatch[2];
        var previewId = previewModelMatch[3];
        // Local preview follows the registry; official scope bypasses same-id overrides.
        if (!previewOfficialOnly && _tryServeCustomSkinResource('/res/pet/' + previewKind + '/' + previewId + '.swf', res, true)) {
          return;
        }
        var previewPath = previewKind === 'fight'
          ? '/resource/fightResource/pet/swf/' + previewId + '.swf'
          : (previewKind === 'skill'
            ? '/resource/fightResource/skill/swf/' + previewId + '.swf'
          : (previewKind === 'icon'
            ? '/resource/pet/head/' + previewId + '.swf'
            : '/resource/groupFightResource/pet/' + previewId + '.swf'));
        var previewReq = https.request({
          hostname:'seer.61.com', port:443, method:req.method === 'HEAD' ? 'HEAD' : 'GET', path:previewPath,
          headers:{ 'User-Agent':'Mozilla/5.0 Seer2LauncherSkinPreview', 'Accept':'application/x-shockwave-flash,*/*' },
          ...
          var length = parseInt(upstream.headers['content-length'], 10);
          var headers = {
            'Content-Type':'application/x-shockwave-flash',
            'Cache-Control':'private, max-age=300',
            'Access-Control-Allow-Origin':'*',
          ...
          if (req.method === 'HEAD') { upstream.resume(); res.end(); }
          else upstream.pipe(res);
        });
        previewReq.setTimeout(20000, function() { previewReq.destroy(new Error('preview resource timeout')); });
        previewReq.on('error', function(error) {
      ...
      // ── 官方同号覆盖与用户自定义替换────────────────────────────────────
      // Explicit official-id overrides outrank generic replace rules per type:id.
      if (_tryServeCustomSkinResource(routePathForOps, res, false, true)) {
        return;
      }
      ...
      // ── 自定义皮肤 SWF 路由 ────────────────────────────────────────────
      // 普通请求替换规则仍具有更高优先级；未被普通规则命中的精灵资源才进入
      // 自定义皮肤映射，随后也不会再落到 GameCache 或原服务器。
      if (_tryServeCustomSkinResource(routePathForOps, res)) {
        return;
      }
...
module.exports = {
  ...
  setCustomSkins,
  getCustomSkins,
  getCustomSkinSnapshotInfo,
  buildCustomSkinManifestXml,
  buildCustomSkinRoutesXml,
  transformCustomSkinXml,
  setLocalSwfDir,
  loadFilesIntoMemory,
  setBaseDir,
  setUClientPetPreviewResolver,
  startInterceptProxy,
  buildPacScript,
  getProxyPort,
